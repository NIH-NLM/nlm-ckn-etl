...
Every column names the source of its values: ``uberon_`` columns carry what the
ontology asserts, ``hubmap_`` columns what the HuBMAP tables assert, ``audit_``
columns what this report computes, and ``Decision`` is the reviewer's.  The
columns of each sheet, and their sources, are also listed in the workbook's
``Columns`` sheet, written from :data:`COLUMN_SOURCES`.
...
# Column holding the reviewer's selection, and the values offered in it
DECISION_COLUMN = "Decision"
DECISIONS = ["keep-HuBMAP", "keep-UBERON", "defer"]

# Names of the sources a column's values can come from, as displayed
UBERON_SOURCE = "UBERON"
HUBMAP_SOURCE = "HuBMAP"
AUDIT_SOURCE = "Audit"
...
# The columns of each detail sheet, in order, each with the source of its
# values and what it holds.  This is the one definition of the column order:
# the frames are built from it, and it is written to the Columns sheet so a
# reviewer can see where every value came from.
COLUMN_SOURCES = {
    LABEL_CONFLICTS_SHEET: [
        (
            "uberon_term",
            UBERON_SOURCE,
            "Term, as a CURIE, labelled by UBERON and relabelled by HuBMAP",
        ),
        (
            "uberon_label",
            UBERON_SOURCE,
            "Label, or labels, the ontology asserts for the term",
        ),
        (
            "hubmap_label",
            HUBMAP_SOURCE,
            "Label the tables assert, which overrides the UBERON label on load",
        ),
        (
            "audit_differs_by",
            AUDIT_SOURCE,
            "How the labels differ: substantive, or case-or-whitespace",
        ),
        (
            "hubmap_source",
            HUBMAP_SOURCE,
            "HuBMAP tuple file, or files, that is organ table, asserting the label",
        ),
        (
            DECISION_COLUMN,
            REVIEWER_SOURCE,
            "Selection to make: " + ", ".join(DECISIONS),
        ),
    ],
    NEW_PART_OF_SHEET: [
        (
            "hubmap_subject_term",
            HUBMAP_SOURCE,
            "Term, as a CURIE, the tables assert is part of the object term",
        ),
        (
            "subject_label",
            f"{UBERON_SOURCE} or {HUBMAP_SOURCE}",
            "Label of the subject term, from the source named in subject_label_source",
        ),
        (
            "subject_label_source",
            AUDIT_SOURCE,
            f"Source of subject_label: {UBERON_SOURCE}, or {HUBMAP_SOURCE} where "
            "the ontology asserts no label",
        ),
        (
            "hubmap_object_term",
            HUBMAP_SOURCE,
            "Term, as a CURIE, the tables assert the subject term is part of",
        ),
        (
            "object_label",
            f"{UBERON_SOURCE} or {HUBMAP_SOURCE}",
            "Label of the object term, from the source named in object_label_source",
        ),
        (
            "object_label_source",
            AUDIT_SOURCE,
            f"Source of object_label: {UBERON_SOURCE}, or {HUBMAP_SOURCE} where "
            "the ontology asserts no label",
        ),
        (
            "uberon_parents",
            UBERON_SOURCE,
            "Terms, with labels, the ontology asserts the subject term is part of",
        ),
        (
            "hubmap_source",
            HUBMAP_SOURCE,
            "HuBMAP tuple file, or files, that is organ table, asserting the edge",
        ),
        (
            DECISION_COLUMN,
            REVIEWER_SOURCE,
    ...
    UNKNOWN_TERMS_SHEET: [
        (
            "hubmap_term",
            HUBMAP_SOURCE,
            "Term, as a CURIE, the tables mention",
        ),
        (
            "hubmap_label",
            HUBMAP_SOURCE,
            "Label, or labels, the tables assert for the term",
        ),
        (
            "audit_reason",
            AUDIT_SOURCE,
            "Why the term is reported: absent-from-uberon, or deprecated",
        ),
        (
            "hubmap_source",
            HUBMAP_SOURCE,
            "HuBMAP tuple file, or files, that is organ table, mentioning the term",
        ),
        (
            DECISION_COLUMN,
            REVIEWER_SOURCE,
...
def columns_of(sheet_name):
    """Return the column names of a detail sheet, in order."""
    return [column for column, _, _ in COLUMN_SOURCES[sheet_name]]


def column_legend():
    """Return the Columns sheet: every detail column, with the source of its values."""
    return pd.DataFrame(
        [
            (sheet_name, column, source, description)
            for sheet_name, columns in COLUMN_SOURCES.items()
            for column, source, description in columns
        ],
        columns=["Sheet", "Column", "Source", "Holds"],
    )
...
def _display_label(uberon, hubmap, term):
    """Return the label of a term, and the name of the source asserting it.

    The UBERON label is preferred, falling back to the HuBMAP label, so a
    reviewer is never shown a HuBMAP label as though the ontology asserted it.

    Returns
    -------
    tuple[str, str]
        The label, and :data:`UBERON_SOURCE` or :data:`HUBMAP_SOURCE`, or a
        pair of empty strings where neither asserts a label.
    """
    label = _uberon_label(uberon, term)
    if label:
        return label, UBERON_SOURCE
    label = _join(hubmap["labels"].get(term, {}).keys())
    return (label, HUBMAP_SOURCE) if label else ("", "")
...
def find_label_conflicts(uberon, hubmap):
    ...
    for term, hubmap_labels in hubmap["labels"].items():
        ...
        for hubmap_label, sources in hubmap_labels.items():
            ...
            rows.append(
                {
                    "uberon_term": curie_of_term(term),
                    "uberon_label": _join(uberon_labels),
                    "hubmap_label": hubmap_label,
                    "audit_differs_by": differs_by,
...
def find_new_part_of_edges(uberon, hubmap):
    ...
    ontology.  Labels are the UBERON ones, falling back to the HuBMAP label
    where the ontology asserts none; the ``*_label_source`` columns name which
    of the two each label came from.
    ...
    for (s_term, o_term), sources in hubmap["part_of"].items():
        ...
        if o_term in uberon_parents:
            continue
        s_label, s_label_source = _display_label(uberon, hubmap, s_term)
        o_label, o_label_source = _display_label(uberon, hubmap, o_term)
        rows.append(
            {
                "hubmap_subject_term": curie_of_term(s_term),
                "subject_label": s_label,
                "subject_label_source": s_label_source,
                "hubmap_object_term": curie_of_term(o_term),
                "object_label": o_label,
                "object_label_source": o_label_source,
    ...
    return pd.DataFrame(
        rows,
        columns=columns_of(NEW_PART_OF_SHEET),
    ).sort_values(["hubmap_subject_term", "hubmap_object_term"], ignore_index=True)
...
def find_unknown_terms(uberon, hubmap, deprecated_terms=None):
    ...
    for term, sources in hubmap["terms"].items():
        ...
        for reason in reasons:
            rows.append(
                {
                    "hubmap_term": curie_of_term(term),
    ...
    return pd.DataFrame(
        rows,
        columns=columns_of(UNKNOWN_TERMS_SHEET),
    ).sort_values(["audit_reason", "hubmap_term"], ignore_index=True)
...
def _format_sheet(worksheet, data_frame):
    ...
    for i_column, column in enumerate(data_frame.columns, start=1):
        ...
        if column == DECISION_COLUMN and len(data_frame) > 0:
...
def write_workbook(sheets, output_path, summary=None):
    ...
    Notes
    -----
    A ``Columns`` sheet is written after the summary, naming the source of the
    values in every column of every detail sheet.
    ...
    summary_rows.append(("Column sources", f"See the {COLUMNS_SHEET} sheet"))
    summary_frame = pd.DataFrame(summary_rows, columns=["Item", "Value"])
    legend_frame = column_legend()
    ...
    with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
        ...
        legend_frame.to_excel(writer, sheet_name=COLUMNS_SHEET, index=False)
        _format_sheet(writer.sheets[COLUMNS_SHEET], legend_frame)
...
def audit(uberon_tuples_path, hubmap_tuple_paths, output_path):
    ...
    summary = [
        ...
        # The names the hubmap_source columns report
        ("HuBMAP sources", _join(Path(p).stem for p in hubmap_tuple_paths)),
